"""
Build the new 2_Job_Rankings.py with landing-first flow.
Reads the original file for the candidate detail panel, then writes the new file.
"""

import os

# ── 1. Read original file to extract detail panel ─────────────────────────
orig_path = r"F:\Projects\resume_ranking\resume_app\pages\2_Job_Rankings.py"
with open(orig_path, "r", encoding="utf-8") as f:
    original = f.read()

# Extract the candidate detail panel (from "row_idx = selected_rows[0]" to just before "# ── Errors")
marker_start = "    row_idx  = selected_rows[0]"
marker_end   = "# ── Errors"
...
if start_idx == -1 or end_idx == -1:
    raise ValueError(f"Could not find detail panel markers: start={start_idx}, end={end_idx}")

detail_panel = original[start_idx:end_idx]

# Indent the detail panel properly (it should be 8 spaces in the new file context)
# The original uses 4-space or 8-space indentation depending on context.
# In the new file, it will be inside `else:` block at 8-space indent.
lines = detail_panel.split("\n")
# Check first non-empty line to determine base indent
base_indent = 0
for line in lines:
...
# The new file needs 8-space indent inside the else block
# Original panel starts at 4-space indent (inside `else:` in original)
# So we need to add 4 more spaces
indented_lines = []
for line in lines:
    if line.strip():
        # Remove original base indent and add 8 spaces
        stripped = line.lstrip()
        new_line = "        " + stripped
...
# Also extract the error expander section
error_section = original[end_idx:]
...
def render_landing():
    ...
    st.markdown(
        ...
        f"Select a department below, then click <b>View Rankings &rarr;</b> on any role to see ranked candidates."
        f"</div>",
        unsafe_allow_html=True,
    )
    ...
    for dept in depts:
        ...
        label = (
            f"{dept_name}  &middot;  "
            f"{dept.get('total_roles', 0)} role(s)  &middot;  "
            f"{dept.get('total_applicants', 0)} applicant(s)  &middot;  "
...
# Insert indented detail panel
new_content += "        " + detail_panel.strip().replace("\n", "\n        ") + "\n"

# Add error section at the end
...
print(f"[OK] Written {output_path} ({len(new_content)} chars, {new_content.count(chr(10))} lines)")

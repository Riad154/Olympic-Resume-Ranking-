...
# ── Section 3: Requirements
    ...
    if "w_edu"        not in st.session_state: st.session_state["w_edu"]        = 15
    if "w_leadership" not in st.session_state: st.session_state["w_leadership"] = 10
...
if submit_clicked:
    if not job_title:
        ...
        st.markdown("**To start ranking, run this command in your terminal:**")
        ...
        st.info("💡 The ranker runs in the background. Refresh Ranking Results to see progress as candidates are processed.")

        # Navigate to rankings
        if st.button("Go to Ranking Results →", type="primary"):
